import { state, setRequest, markDirty, vars } from "./state.js";
...
export function effectiveURL() {
  return paramsToURL(state.request.url, state.request.params, vars());
}
...
export function syncParamsFromURL() {
  ...
  if (p.length) {
    setRequest({ params: p }, { history: false });
  }
}

export function bodyContentType() {
  const type = state.request.body ? state.request.body.type : "";
...
export function ensureContentType() {
  ...
  setRequest({ headers: hs }, { history: false });
}

export function renderRequest() {
  const r = state.request;

  const tabs = ["params", "auth", "headers", "body", "scripts"];
          ...
          .map(function (t) {
            ...
            const count =
              t === "params" && r.params.length
                ? ' <span class="text-muted">(' + r.params.length + ")</span>"
                : "";
...
function renderPane() {
  ...
  if (state.ui.requestTab === "params") {
    return rows("params", r.params, ["Key", "Value", "Description"]);
  }

  if (state.ui.requestTab === "headers") {
    return rows("headers", r.headers, ["Header", "Value"]);
  }
...
function rows(kind, arr, heads) {
  ...
  const addLabel = isParams ? "parameter" : "header";
  ...
  const rowsHtml = arr
    .map(function (x) {
      const opacity = x.enabled === false ? "opacity-50" : "";
...
function authPane(a) {
  ...
  if (a.type === "bearer") {
    extra = field("Token", a.token, "auth-token", "password");
  }

  if (a.type === "basic") {
    extra =
      field("Username", a.username, "auth-user") +
      field("Password", a.password, "auth-pass", "password");
  }

  if (a.type === "apikey") {
    extra =
      field("Key", a.key, "auth-key") +
      `
  ...
  if (a.type === "oauth2") {
    extra =
      ...
      ` +
      field("Authorization URL", a.authorizationUrl, "oauth-auth") +
      field("Token URL", a.tokenUrl, "oauth-token") +
      field("Client ID", a.clientId, "oauth-client") +
      field("Client Secret", a.clientSecret, "oauth-secret", "password") +
      field("Scope", a.scope, "oauth-scope");
  }
...
export function bindRequest(root) {
  root.addEventListener("click", function (e) {
    const tabEl = e.target.closest("[data-req-tab]");

    const tab = tabEl ? tabEl.dataset.reqTab : null;
    ...
    const addEl = e.target.closest("[data-row-add]");

    const add = addEl ? addEl.dataset.rowAdd : null;

    if (add) {
      addRow(add);
    }

    const deleteEl = e.target.closest("[data-row-delete]");

    const del = deleteEl ? deleteEl.dataset.rowDelete : null;
    ...
    const bodyTypeEl = e.target.closest("[data-body-type]");

    const bt = bodyTypeEl ? bodyTypeEl.dataset.bodyType : null;
  ...
  root.addEventListener("change", function (e) {
    const rowEl = e.target.closest("[data-row-field]");

    const f = rowEl ? rowEl.dataset.rowField : null;
  ...
  root.addEventListener("input", function (e) {
    ...
    if (state.request.body.type === "json" && err) {
      try {
...
function formatBody() {
  ...
  try {
    const x = JSON.parse(state.request.body.content);
...
export function prepareRequest() {
  const r = structuredClone(state.request);

  const v = vars();

  r.url = resolveVars(r.url, v);

  r.params = r.params
    .filter(function (p) {
      return p.enabled !== false && p.key.trim();
    })
    .map(function (p) {
      return {
        ...p,
  ...
  r.headers = r.headers
    .filter(function (h) {
      return h.enabled !== false && h.key.trim();
    })
    .map(function (h) {
      return {
        ...h,
  ...
  if (r.auth.type === "bearer" && r.auth.token) {
    r.headers.push({
      key: "Authorization",
      value: "Bearer " + resolveVars(r.auth.token, v),
      enabled: true,
    });
  }

  if (r.auth.type === "basic" && r.auth.username) {
    const username = resolveVars(r.auth.username, v);

    const password = resolveVars(r.auth.password || "", v);
  ...
  if (r.auth.type === "apikey" && r.auth.location === "header") {
    r.headers.push({
      key: r.auth.key,
      value: resolveVars(r.auth.value || "", v),
      enabled: true,
    });
  }
  ...
  const unresolved = unresolvedVars(r.url);
