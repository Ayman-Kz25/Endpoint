...
export function generateJavaScriptAxios(request = {}) {
    ...
    if (headers.length) {
        ...
        headers.forEach((header) => {
            lines.push(
                `        ${escape(header.name)}: ${escape(header.value)},`
            );
        });
    ...
    if (
        ...
        supportsBody(normalized.method)
    ) {
        if (isJsonBody(normalized.body)) {
            lines.push(
                `    data: ${formatJsonBody(normalized.body)},`
            );
        } else {
            lines.push(
                `    data: ${escape(normalized.body)},`
            );
        }
    }
...
export function generateCurl(request = {}) {
    ...
    headers.forEach((header) => {
        lines.push(
            `-H '${`${header.name}: ${header.value}`.replace(
                /'/g,
                "'\\''"
            )}'`
        );
...
export function initCodeGenerator({
    ...
    elements.copyButton =
        document.getElementById(copyButtonId);
    ...
    elements.copyButton?.addEventListener(
        ...
        async (event) => {
            ...
            event.currentTarget.dataset.copied = "true";

            setTimeout(() => {
                delete event.currentTarget.dataset.copied;
            }, 1200);
        }
    );
